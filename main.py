...
class OpenGexExporter(bpy.types.Operator, ExportHelper):
    ...
    WriteFloatMap = [write_float_asis, write_float_as_hex]
    ...
    def find_node(self, name):
        for key in self.nodeArray:
            if key.name == name:
                return self.nodeArray[key]
        return None
    ...
    def process_skinned_meshes(self):
        for key, data in self.nodeArray.items():
            if data["nodeType"] == NodeType.GEOMETRY:

                armature = key.find_armature()
                
                if armature:
                    for bone in armature.data.bones: # type: ignore
                        data = self.find_node(bone.name)
                        if data:
                            # If a node is used as a bone, then we force its type to be a bone.
                            data["nodeType"] = NodeType.BONE
    ...
    def AnimationTangentsNonzero(fcurve):
        ...
        if keyCount > 0:
            ...
            if (math.fabs(key - left) > kExportEpsilon) or (
                math.fabs(right - key) > kExportEpsilon
            ):
                return True
    ...
    def ExportNodeTransform(self, node, scene):
        ...
        if (sampledAnimation) or (
            ...
            if deltaPositionAnimated:
                ...
                (math.fabs(deltaTranslation[0]) > kExportEpsilon)
                or (math.fabs(deltaTranslation[1]) > kExportEpsilon)
                or (math.fabs(deltaTranslation[2]) > kExportEpsilon)
            ):
            ...
            if positionAnimated:
                ...
                (math.fabs(translation[0]) > kExportEpsilon)
                or (math.fabs(translation[1]) > kExportEpsilon)
                or (math.fabs(translation[2]) > kExportEpsilon)
            ):
                self.indent_write(b"Translation\n")
                self.indent_write(b"{\n")
                self.indent_write(b"float[3] {", 1)
            ...
            if deltaRotationAnimated:
                ...
                if mode == "QUATERNION":
                    ...
                    if (
                        (math.fabs(quaternion[0] - 1.0) > kExportEpsilon)
                        or (math.fabs(quaternion[1]) > kExportEpsilon)
                        or (math.fabs(quaternion[2]) > kExportEpsilon)
                        or (math.fabs(quaternion[3]) > kExportEpsilon)
                    ):
                        self.indent_write(
            ...
            if deltaScaleAnimated:
                ...
                (math.fabs(deltaScale[0] - 1.0) > kExportEpsilon)
                or (math.fabs(deltaScale[1] - 1.0) > kExportEpsilon)
                or (math.fabs(deltaScale[2] - 1.0) > kExportEpsilon)
            ):
            ...
            if scaleAnimated:
                ...
                (math.fabs(scale[0] - 1.0) > kExportEpsilon)
                or (math.fabs(scale[1] - 1.0) > kExportEpsilon)
                or (math.fabs(scale[2] - 1.0) > kExportEpsilon)
            ):
                self.indent_write(b"Scale\n", 0, structFlag)
                self.indent_write(b"{\n")
                self.indent_write(b"float[3] {", 1)
    ...
    def ExportBoneTransform(self, armature, bone, scene):
        ...
        if (parentBone) and (
            math.fabs(parentBone.matrix_local.determinant()) > kExportEpsilon
        ):
        ...
        if poseBone:
            ...
            if (parentPoseBone) and (
                math.fabs(parentPoseBone.matrix.determinant()) > kExportEpsilon
            ):
    ...
    def ExportBone(self, armature, bone, scene):
        ...
        if nodeRef:
            self.indent_write(structIdentifier[nodeRef["nodeType"]], 0, True)
    ...
    def apply_transforms(self, scene: bpy.types.Scene):
        if self.option_apply_transforms:
            for obj in scene.objects:
                self.select_and_make_active(obj)
    ...
    def execute(self, context):
        ...
        self.nodeArray: dict[bpy.types.Object, NodeInfo] = {}
