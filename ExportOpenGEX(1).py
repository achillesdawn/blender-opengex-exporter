...
class OpenGexExporter(bpy.types.Operator, ExportHelper):
	...
	#***
	option_apply_transforms : bpy.props.BoolProperty(name="Apply Transforms", description="Apply all transforms of all objects in the scene", default=False)
	#***
	...
	def ExportImageNodeTexture(self, image, attrib):
		...
		texture_dir = os.path.abspath(bpy.path.abspath(bpy.context.preferences.addons[__name__].preferences.texture_directory))
	...
	def ExportMaterials(self):
		...
		for materialRef in self.materialArray.items():
			...
			if bsdf:
				...
				self.ExportMaterialParam(bsdf, specular_name, b"specular", flagsColorOrTexture)
				...
				self.ExportMaterialParam(bsdf, emission_name, b"emission", flagsColorOrTexture )
